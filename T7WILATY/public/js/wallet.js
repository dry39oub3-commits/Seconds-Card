...
function getStatusStyle(status) {
    const map = {
        'مكتمل': { color: '#22c55e', bg: 'rgba(34,197,94,0.08)', border: 'rgba(34,197,94,0.2)', icon: 'fa-check-circle' },
        'مرفوض': { color: '#ef4444', bg: 'rgba(239,68,68,0.08)', border: 'rgba(239,68,68,0.2)', icon: 'fa-times-circle' },
        'ملغي': { color: '#ef4444', bg: 'rgba(239,68,68,0.08)', border: 'rgba(239,68,68,0.2)', icon: 'fa-ban' },
        'قيد المراجعة': { color: '#f97316', bg: 'rgba(249,115,22,0.08)', border: 'rgba(249,115,22,0.2)', icon: 'fa-clock' },
        'قيد الانتظار': { color: '#f97316', bg: 'rgba(249,115,22,0.08)', border: 'rgba(249,115,22,0.2)', icon: 'fa-hourglass-half' },
        'مسترد': { color: '#a78bfa', bg: 'rgba(167,139,250,0.08)', border: 'rgba(167,139,250,0.2)', icon: 'fa-undo' },
    };
    return map[status] || { color: '#94a3b8', bg: 'rgba(148,163,184,0.08)', border: 'rgba(148,163,184,0.15)', icon: 'fa-circle' };
}
...
async function loadTransactions(userId) {
    ...
    if (all.length === 0) {
        list.innerHTML = '<p style="text-align:center; color:var(--text-muted); padding:30px;">لا توجد عمليات بعد.</p>';
        return;
    }
    ...
    list.innerHTML = all.map(t => {
        ...
        const statusBadge = `
            <div style="display:flex; align-items:center; gap:6px; margin-bottom:8px;
                        padding-bottom:8px; border-bottom:1px solid ${color}33;">
        ...
        const rejectBlock = t.reject_reason ? `
            <div style="margin-top:8px; background:rgba(239,68,68,0.08);
                        border:1px solid rgba(239,68,68,0.25); border-radius:6px; padding:8px 10px;">
                ...
                <div style="color:#ef4444; font-weight:600; font-size:13px;">${t.reject_reason}</div>
            </div>` : '';

        const noteBlock = (t.note && isAdminEdit) ? `
            <div style="margin-top:8px; background:rgba(96,165,250,0.08);
                        border:1px solid rgba(96,165,250,0.25); border-radius:6px; padding:8px 10px;">
                <div style="color:var(--text-muted); font-size:11px; margin-bottom:4px;">
                    <i class="fas fa-pen" style="color:#60a5fa;"></i> ملاحظة
                </div>
                <div style="color:#60a5fa; font-weight:600; font-size:13px;">${t.note}</div>
            </div>` : '';
        ...
        if (isCharge) {
            ...
            <div style="margin-top:8px; background:${bg};
                border:1px solid ${border}; border-radius:8px; padding:10px 12px; font-size:12px;">
                ${statusBadge}
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="color:var(--text-muted);">🏦 طريقة الدفع</span>
                    <span style="color:var(--text); font-weight:600;">${t.payment_method || '-'}</span>
                </div>
                ${t.sender_phone ? `
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="color:var(--text-muted);">📱 الرقم المرسل منه</span>
                ...
                <div style="margin-top:8px;">
                    <a href="${t.receipt_url}" target="_blank"
                        style="display:inline-flex; align-items:center; gap:6px;
                            background:rgba(96,165,250,0.1); color:#60a5fa; padding:6px 12px;
        ...
        if (isPurchase) {
            ...
            <div style="margin-top:8px; background:${bg};
                ...
                ${statusBadge}
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="color:var(--text-muted);">🛍️ المنتج</span>
                    <span style="color:#f97316; font-weight:600;">${productName || '-'}</span>
                </div>
                ${t.label ? `
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="color:var(--text-muted);">🏷️ الفئة</span>
        ...
        if (receiptSection) {
            ...
            phoneDiv.innerHTML = `
                ...
                <input type="tel" id="wallet-sender-phone"
                    ...
                    onblur="this.style.borderColor='var(--border, #334155)'">
            `;
...
window.closeDepositModal = () => {
    ...
    const phoneInput = document.getElementById('wallet-sender-phone');
    if (phoneInput) phoneInput.value = '';
    document.querySelectorAll('#payment-methods-container > div').forEach(d => {
        d.style.borderColor = 'var(--border, #334155)';
        d.style.background = 'var(--surface-2, #0f172a)';
    });
};
...
async function loadPaymentMethods() {
    ...
    if (error || !methods || methods.length === 0) {
        container.innerHTML = '<p style="color:var(--text-muted); text-align:center; grid-column:1/-1;">لا توجد بوابات دفع متاحة حالياً</p>';
        return;
    }

    container.innerHTML = methods.map(m => `
        <div onclick="selectMethod('${m.name}', '${m.account_number || ''}', this)"
            style="background:var(--surface-2, #0f172a); border:2px solid var(--border, #334155);
            ...
            <div style="font-weight:bold; font-size:14px; color:var(--text);">${m.name}</div>
        </div>
    `).join('');
}
...
window.copyMethodAccount = () => {
    ...
    const btn = document.querySelector('#method-info button');
...
window.submitCharge = async () => {
    ...
    if (!senderPhone || !/^\d{8}$/.test(senderPhone)) { showToast('⚠️ أدخل رقم الهاتف المرسل منه (8 أرقام)'); return; }
