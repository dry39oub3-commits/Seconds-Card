...
window.toggleCode = (orderId, cardCode) => {
            ...
            <div style="display:flex; flex-direction:column; gap:10px;">
                ${codes.length > 0 ? codes.map((code, i) => `
                    <div style="background:#0f172a; border-radius:10px; padding:14px 16px; display:flex; align-items:center; gap:10px;">
                        <span style="color:#94a3b8; font-size:13px; min-width:24px; flex-shrink:0;">${i + 1})</span>
                        <span style="font-family:monospace; font-size:15px; color:#f97316; font-weight:bold; flex:1; text-align:center; letter-spacing:1px; word-break:break-all;">${code}</span>
                ...
                `).join('') : `
                    <p style="text-align:center; color:#64748b; padding:20px;">جاري المعالجة...</p>
                `}
            </div>
    ...
    // حفظ الأكواد للنسخ الجماعي
    window._currentCodes = codes;
};

window.copyAllCodes = () => {
    if (!window._currentCodes) return;
    navigator.clipboard.writeText(window._currentCodes.join('\n'))
        .then(() => alert('✅ تم نسخ جميع الأكواد!'));
};
            ...
            <div style="display:flex; flex-direction:column; gap:10px; max-height:60vh; overflow-y:auto; padding-left:4px;">
                ${codes.length > 0 ? codes.map((code, i) => `
                    <div style="background:#0f172a; border-radius:10px; padding:14px 16px; display:flex; align-items:center; justify-content:space-between; gap:10px;">
                        ...
                        <span style="font-family:monospace; font-size:15px; color:#f97316; font-weight:bold; flex:1; text-align:center; letter-spacing:1px; word-break:break-all;">${code}</span>
                        <button onclick="copyCode('${code.replace(/'/g, "\\'")}')"
                            style="background:#334155; color:white; border:none; padding:6px 12px; border-radius:6px; cursor:pointer; font-size:12px; white-space:nowrap; flex-shrink:0;">
                            <i class="fas fa-copy"></i> نسخ
                        </button>
                    </div>
                `).join('') : `
                    <p style="text-align:center; color:#64748b;">جاري المعالجة...</p>
                `}
            </div>
